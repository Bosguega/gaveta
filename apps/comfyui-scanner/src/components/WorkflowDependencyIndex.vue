            ...
            <div class="workflow-list">
                ...
                <article v-for="workflow in workflowIndex.workflows" :key="workflow.path" class="workflow-row">
                    ...
                    <div v-if="expanded.has(workflow.path)" class="workflow-detail">
                        <p><b>Modelos:</b> <span v-if="workflow.dependencies.length">{{ workflow.dependencies.map(item => item.name).join(' · ') }}</span><em v-else>Nenhum modelo local identificado</em></p>

...
export async function buildWorkflowDependencyIndex(path: string, items: ScannedItem[]): Promise<WorkflowDependencyIndex> {
    return invoke('build_workflow_dependency_index', { path, scannedItems: items });
}
